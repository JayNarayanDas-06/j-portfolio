...
const skills = [
  { name: 'Technical SEO & Website Auditing', level: 90, icon: FileSearch },
  { name: 'Keyword Research & Search Intent', level: 92, icon: Search },
  { name: 'On-Page SEO & Content Optimization', level: 88, icon: FileText },
  { name: 'Organic Traffic Growth', level: 85, icon: TrendingUp },
  { name: 'SEO Analytics & Reporting', level: 90, icon: BarChart3 },
  { name: 'Competitor & Market Research', level: 87, icon: Target },
  { name: 'Social Media Strategy', level: 82, icon: Users },
  { name: 'Content Planning & Scripting', level: 85, icon: Lightbulb },
  { name: 'Trending Content Research', level: 88, icon: Zap },
  { name: 'Google Analytics & Search Console', level: 91, icon: LineChart },
];
// ===== END EDITABLE TEXT =====
      ...
      <div className="section-container py-[40px]" ref={ref}>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {skills.map((skill, index) => (
            <motion.div key={skill.name} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.1 + index * 0.05 }} className="p-5 rounded-xl bg-card border border-border card-hover group">

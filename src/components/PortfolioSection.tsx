...
const featuredProject = {
  ...
  // Set to a favicon URL or null/empty to show default globe icon
  faviconUrl: '',
};
...
const comingSoonText = 'More projects coming soon.';
const comingSoonHighlight = 'Stay tuned!';
// ===== END EDITABLE TEXT =====
      ...
      <div className="section-container py-[40px]" ref={ref}>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 overflow-hidden border border-border">
                    {featuredProject.faviconUrl ? (
                      <img src={featuredProject.faviconUrl} alt="Project favicon" className="w-8 h-8 object-contain" />
                    ) : (
                      <Globe className="w-6 h-6 text-primary" />
                    )}
                  </div>
                  <div>
                    <span className="text-sm text-primary font-medium">{featuredProject.label}</span>
                    <h3 className="text-2xl md:text-3xl font-bold mt-1">{featuredProject.title}</h3>
                  </div>
                </div>
                <Button variant="outline" className="rounded-full gap-2" asChild>
                  <a href={featuredProject.websiteUrl} target="_blank" rel="noopener noreferrer">
                    {featuredProject.websiteLabel}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.8 }} className="text-center mt-12">
          <p className="text-muted-foreground">
            {comingSoonText} <span className="text-primary">{comingSoonHighlight}</span>
          </p>
        </motion.div>
      </div>
    </section>

...
export const Navigation = () => {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            >
              {logoUrl ? (
                ...
                <>
                  <EditableText value={content.footer.brandName} onChange={(v) => updateSection('footer', { brandName: v })} className="text-foreground" />
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.2 }} className="fixed inset-0 z-40 md:hidden">
              ...
              <div className="flex flex-col items-center gap-4 p-8">
                {navLinks.map((link, index) => (
                  <motion.a
                    ...
                    className={`text-xl font-medium ${activeSection === link.href.replace('#', '') ? 'text-primary' : 'text-muted-foreground'}`}
                  >
                    {link.name}
                  </motion.a>
                ))}
